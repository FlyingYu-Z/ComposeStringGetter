 ...
 *
 */
class ManagementDialog(private val project: Project) : DialogWrapper(project) {

    private val loadedProject: LoadedProject = ProjectPool.getLoadedProjectOrCreate(project)
                    ...
                    Surface(modifier = Modifier.background(Color.Transparent)) {
                        Column(modifier = Modifier.fillMaxSize()) {
                            Column(modifier = Modifier.fillMaxSize().weight(1.0f)) {
                                MappingItemHeader()
                                val list = loadedProject.getProjectConfig()?.mappings ?: emptyList()
                                ...
                                Button(modifier = Modifier.align(Alignment.CenterVertically),
                                    onClick = {
                                        if (!loadedProject.enabled()) {
                                            YesNoDialog(project, "Tip", "You need to set i18n Configuration first") {
                        ...
                        Clickable(modifier = Modifier.align(Alignment.CenterVertically),
                            ...
                            onClick = {
                                YesNoDialog(project, "Tip", "Do you want do delete it?") {
                                    loadedProject.removeMapping(item)
                                }.show()
                            }) {
                            Icon(
